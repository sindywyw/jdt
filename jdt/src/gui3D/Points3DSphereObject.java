...
public class Points3DSphereObject implements GraphNodePicker {
	...
	public Points3DSphereObject(Graphics3DEngine engine, Iterator<Point_dt> points_iter,int num_of_points, delaunay_triangulation.BoundingBox bound_box)
	{
		_3d_engine = engine;
	...
	public void setPointSize(delaunay_triangulation.BoundingBox bound_box)
	{
		int max_point_size = 20;
		int min_point_size = 4;
		double bound_box_size = bound_box.getMaxPoint().x() - bound_box.getMinPoint().x();
	...
	public void notify_graph_node_picked(SceneGraphPath node_path)
	{
		...
		if (node instanceof TransformGroup)
		{
			...
			_3d_engine.point_picked_callback(new Point3d(transform_vec.getX(), transform_vec.getY(), transform_vec.getZ()));
		}
	}
	 ...
	 */
	private void createSpheres(Iterator<Point_dt> points_iter,int num_of_points)
	{
		...
		while ((i<num_of_points) && (points_iter.hasNext()))
		{
	        ...
	        Point_dt currp = points_iter.next();
	        translate.set( (float)currp.x(), (float)currp.y(), (float)currp.z());
